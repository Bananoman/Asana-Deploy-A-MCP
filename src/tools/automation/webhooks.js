...
module.exports = (client) => [
  ...
  {
    name: 'create_webhook',
    description: 'Create a webhook to receive real-time notifications when a resource changes. CRITICAL: target URL MUST be HTTPS and publicly accessible. Asana performs a handshake on creation — sends POST with X-Hook-Secret header, your server must respond 200 with the same X-Hook-Secret in the response header. Heartbeat every 8 hours; auto-deleted after 24 hours without successful heartbeat. Use filters to narrow events: resource_type ("task", "project", "story"), resource_subtype (e.g., "milestone"), action ("changed", "added", "removed", "deleted"), fields (["completed", "assignee"]). Field-level filtering NOT supported for workspace/team/portfolio webhooks. Without filters, ALL changes trigger delivery. Failed deliveries retried with exponential backoff. Related: list_webhooks to see existing, get_webhook to check status, update_webhook to change filters, delete_webhook to remove.',
